...
| TTS provider abstraction | v22.py:920-1035 (`_NullTTSProvider`, `_SayTTSProvider`, `_Pyttsx3Provider`, `_build_tts_provider()`) | macOS `say`, `pyttsx3` | Duck-typed `.speak/.stop/.is_speaking` backends | ✓ | | Unknown |
...
| Function | Source File | Dependencies | Responsibility | C | S | Status |
|---|---|---|---|---|---|---|
| TTS interrupt signaling | v22.py:727-735 `_tts_interrupt` | `threading.Event` | Signals in-progress TTS to stop | ✓ | | Unknown |
| TTS keyboard control (stop) | `src/ui/keyboard.py` via `RuntimeContext.tts`/`.tts_interrupt_event` | KeyboardController, TTS provider | Operator-triggered TTS stop | ✓ | | Unknown |
...
| Client-side TTS/audio output playback | A stub (`_NullTTS`) and unused `--tts`/`--output-device` flags exist; no playback implementation found |
...
3. **Runtime Client validation** — end-to-end audio→WS→server round trip; add test coverage; resolve the TTS/output-device stub.
